...
var date = new Date();
console.log(date);var famousQuote = "The only way to do great work is to love what you do.";
var famous_person = "Steve Jobs";
var message = 'Famous Person: "' + famousQuote + '" - ' + famous_person;
console.log(message);
...
const dinnerGuests: string[] = ["Nasreen Iftikhar", " Kishwe Salman  ", " Talat ","Batool", "Fatima", "Sadaf"];
...
for (var numberwithordinal of numbers) {
 ...
 //question#34
 const 
 favoritePizzas: string[] = ["Pepperoni", "Fajita", "Supreme"];
...
// question#36
function make_shirt_custom(size: string, message: string) {
    console.log("The size of the shirt " + size + " and it will be printed Coller of the \"" + message + "\".");
}

make_shirt_custom("XL", "T-shirt");

// question# 37

function make_shirt_default(size?: string, message?: string) {
    if (size === undefined) { size = "large"; }
    if (message === undefined) { message = "I love TypeScript"; }
    console.log("The shirt size is " + size + " and it will be printed on TypeScript \"" + message + "\"");
}

make_shirt_default("xl"); 
make_shirt_default("lg"); 
make_shirt_default("med"); 

//Question #38

function countryin_city(city: string, country: string) {
    console.log("".concat(city, " is a city in ").concat(country));
}
countryin_city("Karachi", "Pakistan");
function countrywith_city(city: string, country: string) {
    console.log("".concat(city, " is a city in ").concat(country));
}
countrywith_city("Istanbul", "Turkey");
countrywith_city("Texas", "United States");
countrywith_city("Tokyo", "Japan");

//question#39
function cityin_country(city: string, country: string) {
    return "".concat(city, ", ").concat(country);
}
var Locations = cityin_country("Karachi", "Pakistan");
...
console.log(album1); // Output: { artist: 'Artist1', title: 'Album 1' }
console.log(album2); // Output: { artist: 'Artist2', title: 'Album 2', tracks: 12 }
...
function MagiciansShow(magicians: string[]) {
    for (var i = 0; i < magicians.length; i++) {
...
var greatMagicians = GreatMagicians(magicians);
MagiciansShow(greatMagicians);


//Question#43
function createGreatMagicians(magicians: string[]): string[] {
...
function displayMagicians(magicians: string[]): void {
    const magiciansString = magicians.join(", ");
    console.log(magiciansString);
}

const originalMagicians = ["David Copperfield", "David Blaine", "Criss Angel", "Lance Burton", "Doug Henning"];
const modifiedMagicians = createGreatMagicians(originalMagicians);

console.log("Original Magicians:");
displayMagicians(originalMagicians);
...
makeSandwich("Beef", "Cheese", "Lettuce", "Tomato");
makeSandwich("Chicken", "Swiss Cheese", "Pickles");
makeSandwich("peanut Butter", "Jelly");

//question#45
function createCar(manufacturer: string, model: string, ...options: (string | number)[]) {
